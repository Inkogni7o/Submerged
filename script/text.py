...
class Text:
    def __init__(self, text: str, tap: bool):
        self.text = textwrap.wrap(text, 40)
        self.text = self.text + ['Нажмите, чтобы продолжить'] if tap else self.text
    ...
    def draw(self, screen):
        pygame.draw.rect(screen, (181, 184, 177, 50), (0, SIZE[1] - 250, SIZE[0], 250))
        if not self.end:
            if self.timer == 0:
                self.sym_index += 1
            ...
            for i in range(self.string_index + 1):
                ...
                screen.blit(text, (50, SIZE[1] - 230 + 54 * i))
        else:
